...
`{lyrics_hint}` の「グリッド行の直前に置き、`|` 区切りで小節と1:1対応」という仕組みは、歌詞専用ではなく**小節注釈ディレクティブ**の一般形として実装する。パーサ／ジェネレータは「グリッド行に先行する注釈行の集合を直下の行の小節へ位置対応させる」処理を汎用に持ち、`lyrics_hint` はその一登録とする。複数の注釈行が同じグリッド行に先行してよい。これにより、将来のストロークパターン（例: `{stroke: D DU UDU | D D DU}` → `Measure.strokeHint`）などはディレクティブ名の登録とフィールド追加のみで足せる。ストローク機能自体は本リファクタの対象外。

切り分けの原則: **小節に付く情報**（歌詞・ストローク）は注釈ディレクティブ、**コードに付く情報**（押弦・ボイシング）はコード名へのひもづけで表現する。ボイシング変化形は標準 ChordPro の `{define: G-walk base-fret 3 frets ...}` による曲ローカル定義とアプリ辞書（`lib/chords/dictionary.ts`）の2層で解決し（曲ローカル優先）、グリッドのセルには定義名をそのまま書く。セルのコード名は任意文字列のため、この拡張も形式・モデルの変更なしに追加できる。押弦機能自体も本リファクタの対象外。
